...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  router.post('/v1/pushnotifications/subscribe/', function(req, res) {
    var server = getServer(req, res);
//    getServerWithAuth(req, res, function(server) {
...
//    });
  ...
  router.post('/v1/pushnotifications/unsubscribe/', function(req, res) {
    var server = getServer(req, res);
//    getServerWithAuth(req, res, function(server) {
      server.pushNotificationsUnsubscribe(req.body, function(err, response) {
        if (err) return returnError(err, res, req);
        res.json(response);
      });
//    });
  ...
  router.get('/v1/pushnotifications/users/:user/associations', function(req, res) {
    var server = getServer(req, res);
//    getServerWithAuth(req, res, function(server) {
      var opts = {
        user: req.params['user']
      };
      server.pushNotificationsGetUserAssociations(opts, function(err, response) {
        if (err) return returnError(err, res, req);
        res.json(response);
      });
//    });
  ...
  router.get('/v1/pushnotifications/send', function(req, res) {
    getServerWithAuth(req, res, function(server) {
      server.pushNotificationsSend(req.body, function(err, response) {
        if (err) return returnError(err, res, req);
        res.json(response);
      });
    });
