...
WalletService.prototype.pushNotificationsSubscribe = function(opts, cb) {
  ...
  opts.user =  opts.user || self.walletId + '$' + self.copayerId;
  ...
  request({
    ...
    if (!response.statusCode || (response.statusCode && response.statusCode != 200)) {
      var error = {
...
WalletService.prototype.pushNotificationsGetUserAssociations = function(opts, cb) {
  var self = this;

  var user = opts.user || self.walletId + '$' + self.copayerId;

  request({
    url: config.pushNotificationsOpts.pushServerUrl + '/users/' + user + '/associations',
    method: 'GET',
    json: true
  }, function(err, response) {
    if (err || (response.statusCode && response.statusCode != 200)) {
      return cb('Could not get user associations (code: ' + response.statusCode + ')', null);
    }
    return cb(null, response.body.associations);
  });
};

WalletService.prototype.pushNotificationsSend = function(opts, cb) {
  var self = this;

  opts.user = opts.user || self.walletId + '$' + self.copayerId;

  request({
    url: config.pushNotificationsOpts.pushServerUrl + '/send',
    method: 'POST',
    json: true,
    body: opts
  }, function(err, response) {
